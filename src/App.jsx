...
export default function App() {
  ...
  const [config, setConfig] = useState({
    ...
    exportWithBg: true, adminPin: 'padel2026', superAdminPin: 'super2026', 
  ...
  const shareResultAsImage = async (match, team1, team2, action) => {
    ...
    await drawCanvasBg(ctx, canvas, config);
            ...
            <div className="animate-in fade-in duration-500">
               ...
               <div className="space-y-12">
                 ...
                 {tournaments.filter(t => torneosTab === 'activos' ? !t.isArchived : t.isArchived).map(tournament => (
                     <PublicTournamentCard key={tournament.id} tournament={tournament} config={config} allTeams={teams} onTeamClick={openTeamDetails} handleShareCanvas={handleShareCanvas} /> 
                 ))}
               </div>
            </div>
          )}
...
// --- SUBCOMPONENTE: MODAL DETALLE DE JUGADOR ---
function PlayerDetailModal({ player, matches, teams, onClose, config }) {
  
  const pTeams = teams.filter(t => t.player1Id === player.id || t.player2Id === player.id || t.player3Id === player.id).map(t => t.id);
  const pMatches = matches.filter(m => m.status === 'completed' && (pTeams.includes(m.team1Id) || pTeams.includes(m.team2Id)));
  const played = pMatches.length;
  const won = pMatches.filter(m => pTeams.includes(m.winnerId)).length;
  const winrate = played > 0 ? Math.round((won / played) * 100) : 0;

  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
       <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={onClose}></div>
       <div className="theme-bg-card border border-white/10 w-full max-w-md rounded-3xl shadow-2xl relative z-10 overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar">
          <button onClick={onClose} className="absolute top-4 right-4 opacity-50 hover:opacity-100 bg-black/20 hover:bg-black/40 rounded-full p-2 transition-colors z-20"><X size={20} /></button>
          <div className="pt-10 pb-6 px-6 text-center border-b border-white/5 relative" style={{ background: `linear-gradient(to bottom, ${config.primaryColor}33, transparent)` }}>
             <div className="w-28 h-36 mx-auto bg-black/40 rounded-xl flex items-center justify-center border-4 border-black/50 shadow-xl overflow-hidden mb-4 relative z-10">
                {player.photoUrl ? <img src={player.photoUrl} className="w-full h-full object-cover" alt="foto"/> : <Users size={40} className="opacity-40" />}
             </div>
             <h2 className="text-3xl font-black tracking-tight drop-shadow-md theme-font-secondary leading-none">{player.name}</h2>
             
             {player.nickname && <div className="mt-1 theme-primary-text font-black text-lg drop-shadow-md">"{player.nickname}"</div>}
             {player.position && <div className="mt-2 inline-block bg-black/40 px-3 py-1 rounded text-xs uppercase font-bold border border-white/5 opacity-80">{player.position}</div>}
             
             <div className="mt-3"><span className="inline-block theme-primary-bg font-black px-4 py-1 rounded-full text-sm uppercase shadow text-black">{player.category}</span></div>
          </div>
          
          <div className="p-6 theme-text space-y-4">
            
            {config.feature_medals !== false && (
                <div className="bg-black/20 p-5 rounded-xl border border-white/5">
                   <h3 className="text-sm font-black theme-primary-text uppercase tracking-widest mb-4 flex items-center justify-center theme-font-secondary"><Trophy size={16} className="mr-2" /> Logros Destacados</h3>
                   {Array.isArray(player.achievements) && player.achievements.length > 0 ? (
                      <div className="space-y-3">
                         {player.achievements.map((ach, idx) => {
                            let colorClass = "text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.6)]";
                            if(ach.type === 'plata') colorClass = "text-slate-300 drop-shadow-[0_0_8px_rgba(203,213,225,0.5)]";
                            if(ach.type === 'bronce') colorClass = "text-amber-600 drop-shadow-[0_0_8px_rgba(217,119,6,0.6)]";
                            return (
                               <div key={idx} className="flex items-center gap-3 bg-black/40 p-3 rounded-lg border border-white/10">
                                  <Medal size={28} className={colorClass} />
                                  <span className="text-sm font-bold leading-tight">{ach.text}</span>
                               </div>
                            )
                         })}
                      </div>
                   ) : (
                      <p className="opacity-60 text-sm italic text-center">Sin logros registrados aún.</p>
                   )}
                </div>
            )}

            {config.feature_stats !== false && (
              <div className="bg-black/20 p-5 rounded-xl border border-white/5">
                <h3 className="text-sm font-black theme-accent-text uppercase tracking-widest mb-4 flex items-center justify-center theme-font-secondary"><TrendingUp size={16} className="mr-2" /> Estadísticas Históricas</h3>
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div className="bg-black/40 p-3 rounded-lg border border-white/5">
                    <div className="text-2xl font-black theme-primary-text theme-font-secondary">{played}</div>
                    <div className="text-[10px] opacity-60 font-bold uppercase">Jugados</div>
                  </div>
                  <div className="bg-black/40 p-3 rounded-lg border border-white/5">
                    <div className="text-2xl font-black theme-primary-text theme-font-secondary">{won}</div>
                    <div className="text-[10px] opacity-60 font-bold uppercase">Ganados</div>
                  </div>
                  <div className="bg-black/40 p-3 rounded-lg border border-white/5 relative overflow-hidden">
                    <div className="absolute inset-0 theme-accent-bg opacity-10"></div>
                    <div className="text-2xl font-black theme-accent-text theme-font-secondary">{winrate}%</div>
                    <div className="text-[10px] opacity-60 font-bold uppercase">WinRate</div>
                  </div>
                </div>
              </div>
            )}
...
function TeamDetailModal({ team, players, matches, tournaments, onClose, onPlayerClick, config }) {
  ...
  tournaments.forEach(tor => {
    ...
    (tor.zones || []).forEach(z => {
      const stats = z.teams.find(t => t.teamId === team.id);
      if(stats) { playedHere = true; }
    });
    if(playedHere) torneosJugados++;
  });

  const teamMatches = matches.filter(m => m.status === 'completed' && (m.team1Id === team.id || m.team2Id === team.id));
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
       <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={onClose}></div>
       <div className="theme-bg-card border border-white/10 w-full max-w-lg rounded-3xl shadow-2xl relative z-10 overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar">
          <button onClick={onClose} className="absolute top-4 right-4 opacity-50 hover:opacity-100 bg-black/20 hover:bg-black/40 rounded-full p-2 transition-colors z-20"><X size={20} /></button>
          
          <div className="pt-10 pb-6 px-6 text-center border-b border-white/5 relative" style={{ background: `linear-gradient(to bottom, ${config.primaryColor}33, transparent)` }}>
             <div className="w-24 h-24 mx-auto bg-black/40 rounded-full flex items-center justify-center border-4 border-black/50 shadow-xl overflow-hidden mb-4 relative z-10">
                {team.photoUrl ? <img src={team.photoUrl} className="w-full h-full object-cover" alt="logo"/> : <Shield size={40} className="opacity-40" />}
             </div>
             <h2 className="text-3xl font-black tracking-tight drop-shadow-md theme-font-secondary">{team.name}</h2>
             
             {config.feature_stats !== false ? (
                <div className="flex justify-center gap-2 mt-4 flex-wrap">
                   <div className="bg-black/40 px-3 py-1.5 rounded-lg border border-white/5"><div className="text-[10px] opacity-60 font-bold uppercase">Torneos</div><div className="text-lg font-black theme-primary-text leading-none theme-font-secondary">{torneosJugados}</div></div>
                   <div className="bg-black/40 px-3 py-1.5 rounded-lg border border-white/5"><div className="text-[10px] opacity-60 font-bold uppercase">Partidos</div><div className="text-lg font-black theme-primary-text leading-none theme-font-secondary">{played}</div></div>
                   <div className="bg-black/40 px-3 py-1.5 rounded-lg border border-white/5"><div className="text-[10px] opacity-60 font-bold uppercase">Ganados</div><div className="text-lg font-black theme-primary-text leading-none theme-font-secondary">{won}</div></div>
                   <div className="theme-accent-bg/10 px-3 py-1.5 rounded-lg border border-amber-500/30"><div className="text-[10px] opacity-80 font-bold uppercase theme-accent-text">WinRate</div><div className="text-lg font-black theme-accent-text leading-none theme-font-secondary">{winrate}%</div></div>
                </div>
             ) : (
                <div className="flex justify-center gap-4 mt-4">
                   <div className="bg-black/20 px-6 py-2 rounded-xl border border-white/5"><div className="text-xs opacity-60 font-bold uppercase">Torneos Jugados</div><div className="text-xl font-black theme-primary-text theme-font-secondary">{torneosJugados}</div></div>
                </div>
             )}
          </div>

          <div className="p-6 theme-text">
            <h3 className="text-sm font-black opacity-60 uppercase tracking-widest mb-4 text-center">Jugadores (Plantel)</h3>
            <div className="flex justify-center gap-4 md:gap-6">
               <PlayerCardVertical player={p1} size="lg" onClick={() => { onClose(); onPlayerClick(p1?.id); }} config={config} />
               <PlayerCardVertical player={p2} size="lg" onClick={() => { onClose(); onPlayerClick(p2?.id); }} config={config} />
               {p3 && <PlayerCardVertical player={p3} size="lg" title="Suplente" onClick={() => { onClose(); onPlayerClick(p3?.id); }} config={config} />}
            </div>
          </div>
       </div>
    </div>
...
const drawCanvasBg = async (ctx, canvas, config) => {
   if (config.exportWithBg !== false && config.bgUrl) {
      try {
          const img = new Image();
          ...
          ctx.fillStyle = config.bgColor ? config.bgColor + 'E6' : 'rgba(2, 6, 23, 0.9)'; 
          ctx.fillRect(0, 0, canvas.width, canvas.height);
      } catch(e) {
          ctx.fillStyle = config.bgColor || '#0f172a';
          ctx.fillRect(0, 0, canvas.width, canvas.height);
      }
  } else {
      ctx.fillStyle = config.bgColor || '#0f172a'; 
...
const exportZonesAsImage = async (tournament, allTeams, config) => {
  ...
  canvas.width = 1100;
  canvas.height = 350 + (tournament.zones.length * zoneHeaderHeight) + (totalRows * rowHeight) + (tournament.zones.length * 40);
...
const exportBracketAsImage = async (tournament, allTeams, config) => {
  ...
  canvas.height = Math.max(1080, 400 + (maxMatches / 2) * 140 + 100); 
  const ctx = canvas.getContext('2d');
...
function PublicTournamentCard({ tournament, config, allTeams, onTeamClick, handleShareCanvas }) {
    ...
    <div className="theme-bg-card backdrop-blur-md border border-white/10 rounded-3xl p-4 md:p-8 shadow-2xl relative overflow-hidden">
      ...
      {isExpanded && (
         <div className="animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <SymmetricBracket tournament={tournament} allTeams={allTeams} config={config} onTeamClick={onTeamClick} handleShareCanvas={handleShareCanvas} />
         </div>
      )}
    </div>
  );
...
function SymmetricBracket({ tournament, config, allTeams, onTeamClick, isAdmin = false, onEditMatch = null, handleShareCanvas }) {
  const validBrackets = tournament.brackets || [];
  if (validBrackets.length === 0) return null;
  ...
  return (
     <div className="mt-12 relative w-full border-t border-white/10 pt-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 px-2 md:px-4 gap-4">
           ...
           {(handleShareCanvas) && (
             <div className="flex gap-2 w-full md:w-auto">
               <button onClick={async () => { const canvas = await exportBracketAsImage(tournament, allTeams, config); if(canvas) await handleShareCanvas(canvas, `Llaves-${tournament.name}.png`, 'download'); }} className="flex-1 md:flex-none bg-white/10 hover:bg-white/20 font-bold px-3 py-2 rounded-lg flex items-center justify-center transition-colors text-xs md:text-sm shadow-lg">
                 <Download size={14} className="mr-2" /> <span className="hidden sm:inline">Descargar</span>
               </button>
  ...
  return (
    <div>
      ...
      <div className="theme-bg-card backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl">
        ...
        {adminSection === 'torneos' && <AdminTorneos tournaments={tournaments} allTeams={teams} matches={matches} db={db} getCollectionPath={getCollectionPath} config={config} handleShareCanvas={handleShareCanvas} onTeamClick={onTeamClick} />}
  ...
  return (
    <div className="space-y-8">
      {userRole === 'superadmin' && config.feature_stats !== false && (
        <div className="bg-red-500/10 p-5 rounded-2xl border border-red-500/30 flex flex-col md:flex-row justify-between items-center gap-4 animate-in slide-in-from-top-4 shadow-xl">
           <div>
              <h4 className="text-red-400 font-black text-lg flex items-center"><AlertTriangle className="mr-2" /> Peligro: Reiniciar Estadísticas Globales</h4>
              <p className="text-xs text-red-300/80 mt-1 max-w-2xl">Esta acción borrará definitivamente todos los resultados completados. Los jugadores volverán a tener 0 partidos jugados y ganados, y se vaciará el tablero de "Últimos Resultados". No se eliminan torneos ni equipos.</p>
           </div>
           {confirmReset ? (
              <div className="flex flex-col sm:flex-row items-center gap-2 bg-black/40 p-2 rounded-xl border border-red-500/50">
                 <span className="text-xs text-red-400 font-bold px-2">¿Confirmar borrado?</span>
                 <div className="flex gap-2">
                     <button onClick={handleResetStats} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-black shadow-lg transition-colors">Sí, Borrar</button>
                     <button onClick={()=>setConfirmReset(false)} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg text-sm font-bold transition-colors">Cancelar</button>
                 </div>
              </div>
           ) : (
              <button onClick={()=>setConfirmReset(true)} className="bg-red-500/20 text-red-400 border border-red-500/50 px-6 py-3 rounded-xl text-sm font-black hover:bg-red-500 hover:text-white transition-colors whitespace-nowrap shrink-0 shadow-lg">
...
function AdminTorneos({ tournaments, allTeams, matches, db, getCollectionPath, config, handleShareCanvas, onTeamClick }) {
  ...
  return (
    <div className="space-y-10">
      ...
      {tournaments.filter(t => adminTorneosTab === 'activos' ? !t.isArchived : t.isArchived).map(t => (
         <AdminTournamentCard key={t.id} t={t} allTeams={allTeams} matches={matches} db={db} getCollectionPath={getCollectionPath} config={config} handleShareCanvas={handleShareCanvas} onTeamClick={onTeamClick} /> 
      ))}
    </div>
  )
}

function AdminTournamentCard({ t, allTeams, matches, db, getCollectionPath, config, handleShareCanvas, onTeamClick }) {
    ...
    <div className={`bg-black/20 border rounded-2xl p-4 md:p-6 relative transition-all ${t.isArchived ? 'border-amber-500/30' : 'border-white/10'}`}>
      ...
      {isExpanded && (
        <div className="animate-in fade-in slide-in-from-top-4 duration-300">
           ...
           <SymmetricBracket tournament={t} allTeams={allTeams} isAdmin={true} onEditMatch={handleEditMatchOpen} config={config} handleShareCanvas={handleShareCanvas} />
      ...
      <div className="bg-black/20 p-8 rounded-3xl border border-white/10">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="space-y-3 lg:col-span-3">
               <label className="block text-sm font-bold opacity-60">Imagen de Fondo de Pantalla</label>
               <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                     ...
                     <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-xs font-bold opacity-80">Incluir Fondo y Logo en Imágenes Exportadas (Zonas y Llaves)</span>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <>
               <div>
                  ...
                  <div className="flex items-center gap-3">
                     <input type="color" value={form.primaryColor} onChange={e=>setForm({...form, primaryColor: e.target.value})} className="w-10 h-10 bg-transparent border-none cursor-pointer rounded-xl shrink-0" />
                     <input type="text" value={form.primaryColor} onChange={e=>setForm({...form, primaryColor: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-xl p-3 outline-none focus:theme-primary-border" />
                  </div>
               </div>
               ...
               <div>
                  ...
                  <div className="flex items-center gap-3">
                     <input type="color" value={form.titleColor} onChange={e=>setForm({...form, titleColor: e.target.value})} className="w-10 h-10 bg-transparent border-none cursor-pointer rounded-xl shrink-0" />
                     <input type="text" value={form.titleColor} onChange={e=>setForm({...form, titleColor: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-xl p-3 outline-none focus:theme-primary-border" />
                  </div>
               </div>
               ...
               <div>
                  ...
                  <div className="flex items-center gap-3">
                     <input type="color" value={form.accentColor} onChange={e=>setForm({...form, accentColor: e.target.value})} className="w-10 h-10 bg-transparent border-none cursor-pointer rounded-xl shrink-0" />
                     <input type="text" value={form.accentColor} onChange={e=>setForm({...form, accentColor: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-xl p-3 outline-none focus:theme-primary-border" />
                  </div>
               </div>
               ...
               <div>
                  ...
                  <div className="flex items-center gap-3">
                     <input type="color" value={form.textColor} onChange={e=>setForm({...form, textColor: e.target.value})} className="w-10 h-10 bg-transparent border-none cursor-pointer rounded-xl shrink-0" />
                     <input type="text" value={form.textColor} onChange={e=>setForm({...form, textColor: e.target.value})} className="w-full bg-black/40 border border-white/10 rounded-xl p-3 outline-none focus:theme-primary-border" />
                  </div>
               </div>
               ...
               <div>
                  ...
                  <div className="flex flex-col gap-2">
                     <div className="flex items-center gap-3">
                        ...
                        <input type="text" value={form.bgColor} onChange={e=>setForm({...form, bgColor: e.target.value})} placeholder="Ej: rgba(2,6,23,0.9) o #020617" className="w-full bg-black/40 border border-white/10 rounded-xl p-3 outline-none focus:theme-primary-border text-sm" />
                     </div>
